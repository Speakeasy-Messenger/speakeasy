...
- **Vouchflow is the only authentication method.** There is no phone, email, or recovery-code fallback.
...
- [x] **Vouchflow re-attestation** — `NativeVouchflowClient` is wired directly, so every `verify()` reaches the SDK and can mint a replacement device credential. Automatic re-verification backs off from 60 seconds to 15 minutes between completed attempts; user-initiated verification is never throttled.
