...
- [x] Push notifications (notify-only) — `PushProvider` contract + Noop/Mock impls; WS triggers on offline recipient. **FCM/APNs delivery + mobile push-token registration deferred** (need cloud creds + native shells).
...
- [ ] Android FCM registration in mobile native code.
- [ ] 🍎 iOS APNs registration in mobile native code.
- [ ] Mobile uploads its push token to the server via a new `POST /v1/devices/push-token` endpoint (writes to `devices.push_token`). Server-side endpoint is platform-agnostic — ships independently of mobile.
- [ ] Server's `FcmApnsPushProvider` replaces `NoopPushProvider` once `FCM_SERVER_KEY` and APNs auth-key secrets are set on Fly. (Android-FCM half can ship before iOS.)
...
#### 5f. Server polish
- [ ] **Drizzle implementations** of remaining repos — currently only `DrizzleUserRepo` exists; Phase 5 adds Drizzle impls of `PreKeyRepo`, `GroupRepo`, `CommunityRepo`, `MessagesRepo`, `DevicesRepo`. `InMemory*` impls stay as test fixtures.
...
- [ ] **Migration runner** integrated with deploy — `flyctl deploy` triggers migration check; `release_command` in `api.toml` runs `npm run migrate` against the new schema.
- [ ] **Metrics endpoint** — `@fastify/metrics` on `:9091/metrics`, wired to Fly's Prometheus scraper. Uncomment the `[metrics]` block in `infra/fly/api.toml`.
- [ ] **WebSocket production tests** — load test against a 2-instance deploy to confirm cross-instance ack routing under realistic concurrency.
