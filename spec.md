...
> `infra/migrations/0001..0024`. Audited end-to-end against the live
...
- [x] **Replay-safe message IDs** (Phase 5f follow-up, migration `0024_message_delivery_tombstones.sql`). `MessagesRepo.insert` atomically reserves each client-supplied ID in a metadata-only tombstone before persisting the relay payload. Replays before acknowledgement skip recipient fan-out and push; replays after acknowledgement also skip delivery and re-emit `delivered` to a direct sender so its queue can settle. Group/community retries still receive `accepted` while per-recipient fan-out remains single-shot. Regression coverage exercises direct replay before and after ACK, group retry semantics, TTL expiry/reuse, and the migration backfill/trigger contract. Rolling release is intentionally two-phase: `0024` mirrors old-instance inserts/deletes without rejecting old-writer duplicates; a follow-up migration enables the database conflict boundary only after every API instance is tombstone-aware.
