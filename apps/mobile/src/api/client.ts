...
export class ApiClient {
  ...
  /**
   * Background delivery receipt. The headless push handler calls this after it
   * displays a message notification, so the server clears the relay row and
   * does not re-send the push ~45s later (the client's WebSocket is closed
   * while backgrounded, so a WS ack is impossible). Best-effort: a failure just
   * means the server may retry once, which is harmless. Batched.
   */
  async reportDelivered(
    ...
    try {
      await this.doFetch(`${this.baseUrl}/v1/messages/delivered`, {
        method: 'POST',
        headers: {
          'content-type': 'application/json',
          authorization: `Bearer ${deviceToken}`,
      ...
      /* best-effort — the server's retry is the safety net */
    }
  }
