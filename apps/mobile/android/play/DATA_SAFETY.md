...
| Diagnostics                | **Collected** (optional) | NOT shared | The in-app diagnostics buffer (Diagnostics screen) auto-uploads to our error reporting on a crash or a failed call (beta builds only, gated on the `diagStreaming` toggle — default on, user can opt out from the screen). Tied to the account's `userId` server-side. Does NOT include message contents. |
